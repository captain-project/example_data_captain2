...
def generate_init_env(config, seed):
    ...
    config_obj.stop_protecting = time_to_protect
...
# predict
if config["general"]["run_mode"] == "predict":
    protect_m = []
    env_history = []
    ...
    for rep_i in range(parse_str(config["general"]["prediction_runs"])):
        ...
        wNN_params = cn_misc.get_nn_params_from_file(
            os.path.join(config["files"]["models_dir"], config["files"]["trained_model_file"]),
            load_best_epoch=False,
            sample_from_iteration=parse_str(config["files"]["sample_weights_from_last_iter"]),
            seed=SEED + rep_i)
        # ---
        envList = []
        env_train, config_train = generate_init_env(config, SEED * rep_i)
        ...
        env_train.set_calc_reward(True)
        env_train._reward_min_protection = None
        env_train.set_time_to_protect(config_train.stop_protecting + 1)
        env_train.observe()
        envList.append(copy.deepcopy(env_train))

        tmp_res_plot_class = cn_util.plot_map_class(z=config_train.reference_grid_pu_nan, fig_s=[6, 5.5])
        ...
        env_list_optim  = cn.runBatchGeneticStrategyEmpirical(envList,
                                                              ...
                                                              sigma=0.2,
                                                              wNN=wNN_params,
                                                              n_NN_nodes=parse_str(config["policy"]["nn_nodes"]),
                                                              increase_temp=0,
                                                              deterministic=True,
                                                              resolution=np.array([1, 1]),
                                                              max_temperature=1000,
        ...
        species_richness_init = sdm_utils.graph_to_grid(env_optim.bioDivGrid.speciesPerCell(),
                                                        config_train.reference_grid_pu_nan)
        pop_density = sdm_utils.graph_to_grid(env_optim.bioDivGrid.individualsPerCell(), config_train.reference_grid_pu_nan)
        protection_matrix = sdm_utils.graph_to_grid(env_optim.bioDivGrid.protection_matrix, config_train.reference_grid_pu_nan)

        protect_m.append(protection_matrix)
        env_history.append(np.array(env_optim.history))

        protected_range_fraction = env_optim.bioDivGrid.protectedRangePerSpecies() / env_optim.bioDivGrid.geoRangePerSpecies()

        np.savez_compressed(
            file=os.path.join(results_wd, config["files"]["outfile"] + "_stats_%s.npz" % rep_i),
